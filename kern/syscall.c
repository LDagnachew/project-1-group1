...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    if (curenv->env_type == ENV_TYPE_GUEST && e->env_ipc_dstva < (void *) UTOP) {
        ...
        if (((pp = page_lookup(curenv->env_pml4e, srcva, &ppte)) == 0)) {
            return -E_INVAL;
        } 
        if ((!(*ppte & PTE_W) && (perm & PTE_W))) {
            return -E_INVAL;
        } 
        #ifndef VMM_GUEST
            ...
            envid_t guest, void *guest_pa, int perm)
{
    ...
    if (srcva >= (void *)UTOP || (uintptr_t)guest_pa >= guest_env->env_vmxinfo.phys_sz) {
        return -E_INVAL;
    }
    // Next we must check for Alignment & Bounds of guest
